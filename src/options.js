...
function validateOptions(options) {
  if (!Versions[options.version] && !Versions[options.maxVersion]) {
  ...
  if (options.maxProtocolVersion > CURRENT_VERSION) {
    throw new Error(
      `Protocol version < ${CURRENT_VERSION} : ${options.protocolVersion}, too old`
    );
  }
